...
AVCodecContextPtr alloc_codec_context(
  ...
  const AVCodec* codec = [&]() {
    if (decoder_name) {
      ...
      HCODEC_CHECK(c, "Unsupported codec: ", decoder_name.value());
      return c;
    } else {
      const AVCodec* c = avcodec_find_decoder(codec_id);
      HCODEC_CHECK(c, "Unsupported codec: ", avcodec_get_name(codec_id));
      return c;
    }
  }();

  AVCodecContext* codec_ctx = avcodec_alloc_context3(codec);
  HCODEC_CHECK(codec_ctx, "Failed to allocate CodecContext.");
...
enum AVPixelFormat get_hw_format(
  ...
  HCODEC_WARN("Failed to get HW surface format.");
...
#ifndef USE_CUDA
    HCODEC_CHECK(false, "humecodec is not compiled with CUDA support.");
#else
...
KeyType StreamProcessor::add_stream(
  ...
  switch (codec_ctx->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
      post_processes.emplace(
          ...
          std::forward_as_tuple(get_audio_process(
              stream_time_base, codec_ctx, filter_description,
    ...
    case AVMEDIA_TYPE_VIDEO:
      ...
      return current_key++;
    default:
      HCODEC_CHECK(false, "Only Audio and Video are supported");
  }
}
...
void StreamProcessor::set_discard_timestamp(int64_t timestamp) {
  HCODEC_CHECK(timestamp >= 0, "timestamp must be non-negative.");
  discard_before_pts =
