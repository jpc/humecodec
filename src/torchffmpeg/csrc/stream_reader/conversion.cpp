...
void NV12CudaConverter::convert(const AVFrame* src, torch::Tensor& dst) {
  ...
  auto status = cudaMemcpy2D(
      dst.data_ptr(), width, src->data[0], src->linesize[0],
      width, height, cudaMemcpyDeviceToDevice);
  TORCH_CHECK(cudaSuccess == status, "Failed to copy Y plane to Cuda tensor.");
  status = cudaMemcpy2D(
      tmp_uv.data_ptr(), width, src->data[1], src->linesize[1],
      width, height / 2, cudaMemcpyDeviceToDevice);
...
void P010CudaConverter::convert(const AVFrame* src, torch::Tensor& dst) {
  ...
  TORCH_INTERNAL_ASSERT(
      AV_PIX_FMT_CUDA == fmt, "Expected CUDA frame. Found: ", av_get_pix_fmt_name(fmt));
  TORCH_INTERNAL_ASSERT(
      AV_PIX_FMT_P010 == sw_fmt, "Expected P010 format. Found: ", av_get_pix_fmt_name(sw_fmt));

  auto status = cudaMemcpy2D(
      dst.data_ptr(), width * 2, src->data[0], src->linesize[0],
      width * 2, height, cudaMemcpyDeviceToDevice);
  TORCH_CHECK(cudaSuccess == status, "Failed to copy Y plane to CUDA tensor.");
  status = cudaMemcpy2D(
      tmp_uv.data_ptr(), width * 2, src->data[1], src->linesize[1],
      width * 2, height / 2, cudaMemcpyDeviceToDevice);
  TORCH_CHECK(cudaSuccess == status, "Failed to copy UV plane to CUDA tensor.");
...
void YUV444PCudaConverter::convert(const AVFrame* src, torch::Tensor& dst) {
  ...
  TORCH_INTERNAL_ASSERT(
      AV_PIX_FMT_CUDA == fmt, "Expected CUDA frame. Found: ", av_get_pix_fmt_name(fmt));
  TORCH_INTERNAL_ASSERT(
      AV_PIX_FMT_YUV444P == sw_fmt, "Expected YUV444P format. Found: ", av_get_pix_fmt_name(sw_fmt));

  for (int i = 0; i < 3; ++i) {
    auto status = cudaMemcpy2D(
        dst.index({0, i}).data_ptr(), width, src->data[i], src->linesize[i],
        width, height, cudaMemcpyDeviceToDevice);
    TORCH_CHECK(
